...
public class Application {
    ...
    public static void produceMessages(int numberOfMessages, Producer<Long, String> kafkaProducer)
        ...
        for(int i=0; i<numberOfMessages; ++i){
            ...
            //ProducerRecord<Long, String> record = new ProducerRecord<>(TOPIC, key, value); // partition not specified
            ProducerRecord<Long, String> record = new ProducerRecord<>(TOPIC, value); // will be sent in round-robin fashion to partitions

            RecordMetadata recordMetadata = kafkaProducer.send(record).get();
            System.out.println(String.format("Record with (key %s val %s) was sent to (partition %d, offset %d)",
